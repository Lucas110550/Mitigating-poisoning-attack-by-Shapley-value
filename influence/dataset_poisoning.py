...
def iterative_attack(top_model, full_model, top_graph, full_graph, project_fn, test_indices, test_description=None, 
	...
	G = np.copy(top_model.data_sets.train.x)
		...
		if len(test_indices) < 100:
			...
			if ((early_stop is not None) and (len(test_indices) == 1)):
				if test_pred[0, int(full_model.data_sets.test.labels[test_indices])] < early_stop:
					...
					return True, poisoned_X_train_subset, indices_to_poison, G, top_model.data_sets.train.x, top_model.data_sets.train.labels, top_model.data_sets.test.x, top_model.data_sets.test.labels

	return False, poisoned_X_train_subset, indices_to_poison, G, top_model.data_sets.train.x, top_model.data_sets.train.labels, top_model.data_sets.test.x, top_model.data_sets.test.labels
